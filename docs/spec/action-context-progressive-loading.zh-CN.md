...
**Action Context 渐进式加载**利用现有的 Skill 注入机制，在 Agent 执行前将完整的参考材料写入磁盘文件，Agent 在 `SKILL.md` 索引的引导下按需读取。Briefing prompt 精简为摘要 + 引用指针，不再承载完整内容。
...
增强后:
  ...
  ActionContextBuilder ──完整材料──→ skills/action-context/ ──按需读取──→ Agent
```
...
4. **临时生命周期** — action-context 是 per-execution 的一次性目录，执行完自动清理
...
║  │    ExtraSkills:    ["action-signal", "action-context"],       │   ║
║  │    EphemeralSkills: {"action-context": actionContextDir},     │   ║
║  │  })                                                           │   ║
║  │    │                                                         │   ║
║  │    └── Sandbox.Prepare()                                      │   ║
║  │         ├── link action-signal (全局 skill, symlink)           │   ║
║  │         └── link action-context (临时 skill, symlink 到生成目录) │   ║
║  └─────────────────────────────────────────────────────────────┘   ║
║                               │                                     ║
║  ┌────────────────────────────▼────────────────────────────────┐   ║
║  │ Phase C: 发送 Prompt                                         │   ║
...
║  │    → "{精简 BriefingSnapshot}"                                 │   ║
║  │    → "# Reference Materials"                                  │   ║
║  │    → "> 完整材料在 skills/action-context/, 按需读取"            │   ║
║  │    → "# Acceptance Criteria"                                  │   ║
║  │    → "- criterion 1 ..."                                      │   ║
║  └─────────────────────────────────────────────────────────────┘   ║
║                               │                                     ║
...
║  │ Phase D: Agent 执行                                          │   ║
║  │                                                              │   ║
║  │  Agent 启动 → 自动读取 skills/action-context/SKILL.md         │   ║
║  │    → 看到材料索引表                                            │   ║
║  │    → 按需 Read work-item.md / upstream/*.md / ...             │   ║
║  │    → 完成后 signal.sh complete "..."                          │   ║
║  └─────────────────────────────────────────────────────────────┘   ║
║                               │                                     ║
║  defer: Cleanup(actionContextDir)  ← 执行完自动清理临时目录         ║
╚═════════════════════════════════════════════════════════════════════╝
```
...
| 文件 | 来源 | 内容 | 生成条件 |
|------|------|------|----------|
| `work-item.md` | `Store.GetWorkItem()` | WorkItem 完整 Title + Body + Priority + Labels | WorkItem 存在 |
...
| `agent-memory.md` | Agent 跨 Action 记忆 | `CtxAgentMemory` |
...
---
name: action-context
description: Pre-loaded reference materials for the current action execution
---
...
|------|-------------|--------------|
| `work-item.md` | Full work item details (title, body, priority, labels) | At start — understand the full task |
| `upstream/requirements.md` | Full output of upstream action "requirements" | When you need context from this predecessor action |
| `upstream/design.md` | Full output of upstream action "design" | When you need context from this predecessor action |
| `acceptance.md` | Detailed acceptance criteria for this action | Before signaling completion — verify all criteria |
...
**注意**：`action-context` 是临时 skill，不需要额外的 frontmatter 字段；只保留 `name` 和 `description` 即可。
...
| 文件 | 职责 |
|------|------|
| `internal/skills/context_builder.go` | `ActionContextBuilder` — 根据 Action/WorkItem/Deliverables 生成 `action-context` 目录 |
...
// (mounted at runtime as skills/action-context/).
...
// Cleanup removes the action-context directory. Safe to call with empty string.
...
type ACPExecutorConfig struct {
    ...
    // StepContextBuilder is the historical field name.
...
if cfg.StepContextBuilder != nil &&
    ...
    dir, buildErr := cfg.StepContextBuilder.Build(ctx, ctxParentDir, action, exec)
    if buildErr != nil {
...
> Full details (work item body, upstream outputs, feature manifest) are pre-loaded
> in `skills/action-context/`. Read the `SKILL.md` there for an index of
> available files. Read individual files on demand — do not load everything.
```

## 与现有 Briefing 预算的协作

action-context 不改变现有 BriefingBuilder 的逻辑和字符预算。两层并存：

| 层 | 内容 | 字符限制 | 用途 |
|----|------|----------|------|
| **Prompt 层** (BriefingSnapshot) | 摘要：Objective + WorkItem 摘要 + Upstream 摘要 + Manifest 精简 | ≤12000 字符 | Agent 的即时任务理解 |
| **文件层** (`action-context`) | 完整版：WorkItem 全文 + Deliverable 全文 + Manifest 全量 JSON | 无限制 | Agent 按需深入 |

当前阶段 Briefing 预算不变；未来可以考虑在有 action-context 时**主动缩减** Briefing 预算（例如 WorkItem 摘要改为仅 Title），以节省 prompt token。

## 生命周期管理

```
ActionContextBuilder.Build()
    → 写入 .ai-workflow/action-contexts/run-<id>/action-context/
    → Sandbox 链接到 agent home: skills/action-context → 上述目录

Agent 执行中
    → 通过文件系统读取 skills/action-context/ 下的文件
    → 写入的文件为只读（Agent 不应修改）

ACP 执行器 defer
    → Cleanup(actionContextDir)
    → 删除 .ai-workflow/action-contexts/run-<id>/ 整个目录
...
**异常安全**：
- `Build()` 失败 → 不注入 action-context，Agent 仍使用 Briefing prompt（降级）
- 链接失败 → 非致命，slog.Warn 后继续
- 清理失败 → 忽略，目录在 `.ai-workflow/action-contexts/` 下，可手动清理

## 前提条件

- Agent 必须具备 `fs_read` 能力才能读取 action-context 文件
...
| `TestBuild_NoWorkItem` | WorkItem 不存在时跳过 `work-item.md` |
...
| 场景 | 验证点 |
|------|--------|
| E2E Action 执行 | `action-context` 目录在执行前存在，执行后被清理 |
| Agent 实际读取 | Agent 的输出引用了 `action-context` 中的文件内容 |
| 降级模式 | `StepContextBuilder = nil` 时行为与现有完全一致 |
| Session 复用 | 复用会话时 `action-context` 被刷新（新 run 的材料） |
...
| `internal/adapters/executor/acp.go` | **修改** — 注入 `action-context` |
