...
检查项：
1. 每个任务的描述是否清晰到 AI 编码 Agent 可以独立执行
2. 验收标准是否明确、可验证
3. 任务粒度是否合适（太大需要拆分，太小可以合并）
4. 建议的模板（full/standard/quick/hotfix）是否合理
...
> 审核面板的完整配置（`review_panel:`）见 [spec-api-config.md](spec-api-config.md) Section III。
...
type RefinedDetail struct {
    ...
    Acceptance     string     // 明确的验收标准
    EstimatedSteps int        // 预估步骤数（供调度器参考）
}
```

细化结果写入 TaskItem.Description（追加到原描述之后），不改变 TaskItem 的数据结构。
...
| 文件 | 用途 | 更新频率 |
|------|------|---------|
| `task_plan.md` | 当前 TaskItem 的目标、验收标准、文件清单 | 开始时写入，不再更新 |
...
3. dispatchTask(item)
   ...
   │   Pipeline 的 requirements 阶段注入 item.Description 作为需求
   ├── 更新 item.PipelineID
...
> DAG Scheduler 的完整配置（`dag_scheduler:`）见 [spec-api-config.md](spec-api-config.md) Section III。
