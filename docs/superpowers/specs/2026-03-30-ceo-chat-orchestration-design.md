...
ceo task create
ceo task decompose
ceo task assign-profile
ceo task follow-up
ceo task reassign
ceo task escalate-thread
```
...
第一阶段可先落在 metadata 或现有 link 关系上，不强制新增表。
...
> `CEO = Chat Profile + CEO Management Skill + 窄 Management CLI`
