...
// GeneratedStep is the LLM output for a single step in the generated DAG.
type GeneratedStep struct {
	Name                 string   `json:"name"`
	Type                 string   `json:"type"`                            // "exec" | "gate" | "composite"
	DependsOn            []string `json:"depends_on,omitempty"`            // step names this depends on
	AgentRole            string   `json:"agent_role,omitempty"`            // "worker" | "gate" | "lead"
	RequiredCapabilities []string `json:"required_capabilities,omitempty"` // capability tags for agent matching
	AcceptanceCriteria   []string `json:"acceptance_criteria,omitempty"`   // what "done" looks like
	Description          string   `json:"description,omitempty"`           // what this step should accomplish
}

// GeneratedDAG is the LLM output for the full DAG.
